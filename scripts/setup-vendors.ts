...
async function fetchLatestCefVersion(): Promise<string> {
  ...
  if (!platform?.versions?.length) {
    throw new Error(
      "Unexpected CEF index format — check https://cef-builds.spotifycdn.com/index.html manually",
    );
  }
...
async function setupSpout() {
  ...
  // Spout2 releases publish a zip with prebuilt binaries
  // URL format: https://github.com/leadedge/Spout2/releases/download/TAG/Spout_TAG.zip
  // But the actual release assets vary. Let's use the source zip + their prebuilt Binaries dir.
  // The most reliable approach: download the release source and use the Binaries folder.
  ...
  // The Spout2 repo has BUILD/Binaries/ with prebuilt libs organized by CRT variant
  // Structure: BUILD/Binaries/x64/{MT,MD}/{lib,bin}
  // And SPOUTSDK/SpoutDirectX/ for headers
  const binX64 = join(spoutExtracted, "BUILD", "Binaries", "x64");
  const sdkDir = join(spoutExtracted, "SPOUTSDK");
  ...
  // Copy headers
  const includeDir = join(spoutDir, "include");
  mkdirSync(includeDir, { recursive: true });

  // SpoutDX headers
  const spoutDxSrc = join(sdkDir, "SpoutDirectX", "SpoutDX");
  if (existsSync(spoutDxSrc)) {
    copyDirRecursive(spoutDxSrc, join(includeDir, "SpoutDX"));
  }
  // SpoutGL headers (for reference, not used in build)
  const spoutGlSrc = join(sdkDir, "SpoutGL");
  if (existsSync(spoutGlSrc)) {
    copyDirRecursive(spoutGlSrc, join(includeDir, "SpoutGL"));
  }
  // SpoutLibrary headers
  const spoutLibSrc = join(sdkDir, "SpoutLibrary");
  if (existsSync(spoutLibSrc)) {
    copyDirRecursive(spoutLibSrc, join(includeDir, "SpoutLibrary"));
  }

  // Copy MT and MD prebuilt binaries
  for (const variant of ["MT", "MD"]) {
    const src = join(binX64, variant);
    if (existsSync(src)) {
